(() => {
  ...
  const flows={
    ...
    ffn:chain('显微结构：检测候选特征 → 按输入激活 → 重组贡献',[
      ['Y_E','每个位置已有上下文表示；本次 FFN 不读取其他行。'],['W₁：提特征','W₁ 的每一列组合输入分量，配合 b₁ 形成一个候选特征响应。'],['ReLU：开关','非正响应归零，正响应保留原强度；哪些通道通过由当前输入决定。'],['W₂：重组','每个激活强度缩放 W₂ 对应行的输出方向，再求和并加 b₂。'],['F_E','得到主干宽度的贡献向量；随后加回 Y_E，才接入残差主干。']]),
    ...
    decoderffn:chain('显微结构：Decoder 也先检测、再激活、再重组',[
      ['Y₂','已经融合目标前缀与源信息的表示。'],['W₁：提特征','用 Decoder 本层的 W₁、b₁ 计算候选特征响应。'],['ReLU：开关','按当前输入让正响应通过，保留响应强度。'],['W₂：重组','激活强度乘各输出方向，再求和并加 b₂。'],['F','输出贡献随后加回本次 FFN 输入 Y₂。']]),
  ...
  flows.ffndetect={title:'显微结构：W₁ 的不同列检测不同方向',nodes:[N('y','yᵢ',0,1,0),N('w1','点积列 1 + b₁₁',1,0,1),N('w2','点积列 2 + b₁₂',1,1,1),N('wj','点积列 j + b₁ⱼ',1,2,1),N('t1','tᵢ₁',2,0,2),N('t2','tᵢ₂',2,1,2),N('tj','tᵢⱼ',2,2,2)],edges:[E('y','w1'),E('y','w2'),E('y','wj'),E('w1','t1'),E('w2','t2'),E('wj','tj')],steps:['同一位置的全部输入分量，提供给每个候选通道。','不同列对输入作不同的加权组合，并加对应偏置。','每个通道产生一个标量响应，合起来就是 tᵢ；尚未经过 ReLU。']};
  flows.ffngate={title:'显微结构：逐通道开关，正响应保留强度',nodes:[N('t1','2.3',0,0,0),N('t2','−1.7',0,1,0),N('t3','0.8',0,2,0),N('g1','max(0, 2.3)',1,0,1),N('g2','max(0, −1.7)',1,1,1),N('g3','max(0, 0.8)',1,2,1),N('z1','2.3：通过',2,0,2),N('z2','0：关闭',2,1,2),N('z3','0.8：通过',2,2,2)],edges:[E('t1','g1'),E('t2','g2'),E('t3','g3'),E('g1','z1'),E('g2','z2'),E('g3','z3')],steps:['这里的三个数是 W₁、b₁ 已经算出的响应。','对每个分量独立应用相同函数，不做跨通道求和。','第 2 个通道本次无贡献；第 1、3 个分别以原强度进入 W₂。']};
  flows.ffnwrite={title:'显微结构：强度乘输出方向，汇总成贡献',nodes:[N('z','zᵢ：激活强度',0,1,0),N('r1','zᵢ₁ × W₂ 行 1',1,0,1),N('r2','zᵢ₂ × W₂ 行 2',1,1,1),N('rj','zᵢⱼ × W₂ 行 j',1,2,1),N('sum','贡献求和',2,1,2),N('bias','b₂',2,2,0),N('f','相加 → fᵢ',3,1,3)],edges:[E('z','r1'),E('z','r2'),E('z','rj'),E('r1','sum'),E('r2','sum'),E('rj','sum'),E('sum','f'),E('bias','f')],steps:['zᵢ 是当前输入决定的激活；W₂、b₂ 是可训练参数。','每个标量强度乘对应整行方向，产生主干宽度的贡献向量。','把各通道的向量贡献相加；零强度通道在本次求和中贡献零。','加上 b₂ 得到 fᵢ。接下来残差连接会把它加回 yᵢ。']};
